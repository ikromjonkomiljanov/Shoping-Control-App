...
function renderLogRow(l, showEdit) {
  ...
  if (showEdit && l.type === "sale") {
    actions =
      ...
      '<button class="btn btn-sm btn-ghost" style="padding:3px 7px" onclick="showEditSaleModal(\'' +
      l.id +
      "')\">✏</button>" +
      '<button class="btn btn-sm btn-red"   style="padding:3px 7px" onclick="confirmDeleteSale(\'' +
      l.id +
      "')\">🗑</button>" +
      "</div>";
  }
...
function renderDashboard() {
  ...
  var tasks = Storage.getTasks().filter(function (t) {
    return t.status === "active";
  });
  var wsTotal = Object.values(wsInv).reduce(function (s, v) {
    return s + v;
  }, 0);
  var shopTotal = Object.values(shopInv).reduce(function (s, v) {
    return s + v;
  }, 0);
  ...
  $("#products-tbody").innerHTML = products
    .map(function (p) {
      return (
        ...
        p.unit +
        "</span></td>" +
        '<td><span class="badge badge-blue" style="font-size:11px">ID: ' +
        p.id +
        "</span></td>" +
        "<td>" +
        '<button class="btn btn-sm btn-ghost" onclick="showEditProductModal(\'' +
        p.id +
        "')\">✏ Tahrirlash</button> " +
        '<button class="btn btn-sm btn-red"   onclick="confirmDeleteProduct(\'' +
        p.id +
        "')\">🗑</button>" +
        "</td>" +
        "</tr>"
...
function showEditProductModal(id) {
  ...
  openModal(
    mHead("✏ Mahsulotni tahrirlash") +
      ...
      '<input type="text" id="ep-name" value="' +
      p.name +
      '"/></div>' +
      '<div class="form-row cols-2">' +
      '<div><label class="form-label">Emoji</label><input type="text" id="ep-icon" value="' +
      p.icon +
      '" maxlength="4"/></div>' +
      '<div><label class="form-label">Birlik</label><select id="ep-unit">' +
      '<option value="dona"' +
      (p.unit === "dona" ? " selected" : "") +
      ">dona</option>" +
      '<option value="juft"' +
      (p.unit === "juft" ? " selected" : "") +
      ">juft</option>" +
      '<option value="metr"' +
      (p.unit === "metr" ? " selected" : "") +
      ">metr</option>" +
      "</select></div>" +
      "</div>" +
  ...
  $("#ws-inventory-body").innerHTML = products
    .map(function (p) {
      ...
      return (
        ...
        fmtNum(qty) +
        "</span></td>" +
        "<td><button class=\"btn btn-sm btn-ghost\" onclick=\"showAdjustInvModal('ws','" +
        p.id +
        "','" +
        p.name +
        "'," +
...
function showAdjustMatModal(matId, matName, currentQty) {
  openModal(
    mHead("✏ " + matName + " miqdori") +
      '<div class="info-box">Hozirgi miqdor: <strong>' +
      fmtNum(currentQty) +
      "</strong></div>" +
      '<div class="form-group"><label class="form-label">Yangi miqdor</label>' +
      '<input type="number" id="adj-mat-qty" min="0" value="' +
      currentQty +
      '"/></div>' +
...
function showAdjustInvModal(type, productId, productName, currentQty) {
  ...
  openModal(
    mHead("✏ " + title + " — " + productName) +
      ...
      "</strong></div>" +
      '<div class="form-group"><label class="form-label">Yangi miqdor</label>' +
      '<input type="number" id="adj-inv-qty" min="0" value="' +
      currentQty +
      '"/></div>' +
      mActions(
        "Bekor",
        "💾 Saqlash",
        "submitAdjustInv('" + type + "','" + productId + "')",
      ),
  );
}
...
function renderGroupList() {
  ...
  el.innerHTML = groups
    .map(function (g) {
      var gt = tasks.filter(function (t) {
        return t.groupId === g.id;
      });
      var active = gt.filter(function (t) {
        return t.status === "active";
      }).length;
      var done = gt.filter(function (t) {
      ...
      return (
        ...
        g.name +
        "')\">📋 Topshiriq</button>" +
        '<button class="btn btn-sm btn-amber"   onclick="showGroupPasswordModal(\'' +
        g.id +
        "')\">🔑</button>" +
...
function renderTaskCard(t) {
  ...
  var html =
    ...
    (grp
      ...
      : "") +
    (!done
      ? '<button class="btn btn-sm btn-ghost" style="padding:3px 7px" onclick="showEditTaskModal(\'' +
        t.id +
        "')\">✏</button>"
      : "") +
    '<button class="btn btn-sm btn-red" style="padding:3px 7px" onclick="confirmDeleteTask(\'' +
    t.id +
    "')\">🗑</button>" +
    "</div>" +
    "</div>" +
  ...
  if (!done) {
    html +=
      ...
      '<span style="font-size:12px;color:var(--ink3);margin-left:6px">Qoldi: ' +
...
function showCreateGroupModal() {
  openModal(
    mHead("👥 Yangi guruh yaratish") +
      ...
      '<div class="form-group"><label class="form-label">Guruh paroli * <small style="color:var(--ink3)">(ishchi kirish uchun)</small></label>' +
      '<input type="text" id="grp-password" placeholder="Kamida 3 ta belgi"/></div>' +
      '<div class="form-group"><label class="form-label">Rang</label>' +
      colorPickerHtml("#2563eb") +
      "</div>" +
...
function submitCreateGroup() {
  ...
  if (!pw || pw.length < 3) {
    toast("Parol kamida 3 ta belgi", "error");
    return;
  }
  var r = Storage.createGroup(name, color, pw);
  if (!r.ok) {
    toast(r.msg, "error");
    return;
  }
...
function showEditGroupModal(id) {
  ...
  openModal(
    mHead("✏ Guruhni tahrirlash") +
      '<div class="form-group"><label class="form-label">Guruh nomi *</label>' +
      '<input type="text" id="egrp-name" value="' +
      g.name +
      '"/></div>' +
      '<div class="form-group"><label class="form-label">Rang</label>' +
      colorPickerHtml(g.color) +
      "</div>" +
...
function showEditTaskModal(id) {
  ...
  openModal(
    mHead("✏ Topshiriq — " + (prod ? prod.name : "")) +
      '<div class="info-box">Allaqachon ishlab chiqarilgan: <strong>' +
      fmtNum(t.produced) +
      " ta</strong></div>" +
      '<div class="form-row cols-2">' +
      '<div><label class="form-label">Jami miqdor *</label>' +
      '<input type="number" id="et-qty" min="' +
      t.produced +
      '" value="' +
      t.quantity +
      '"/></div>' +
      '<div><label class="form-label">Muddati</label>' +
      '<input type="date" id="et-deadline" value="' +
      (t.deadline || "") +
      '"/></div>' +
      "</div>" +
      '<div class="form-group"><label class="form-label">Narx (1 dona, so\'m)</label>' +
      '<input type="number" id="et-price" value="' +
      t.pricePerItem +
      '"/></div>' +
      '<div class="form-group"><label class="form-label">Izoh</label>' +
      '<textarea id="et-note">' +
...
function confirmDeleteTask(id) {
  ...
  if (!t) return;
  var prod = Storage.getProductById(t.productId);
  if (!confirm('"' + (prod ? prod.name : "Topshiriq") + "\" ni o'chirasizmi?"))
    return;
...
function showProduceModal(taskId) {
  ...
  openModal(
    mHead("🔨 Ishlab chiqarish — " + (prod ? prod.name : "")) +
      ...
      "Topshiriq: <strong>" +
      fmtNum(task.quantity) +
      "</strong> ta &nbsp;|&nbsp; " +
      "Bajarildi: <strong>" +
      fmtNum(task.produced) +
      "</strong> ta &nbsp;|&nbsp; " +
      'Qoldi: <strong style="color:var(--blue)">' +
      fmtNum(remaining) +
      "</strong> ta" +
      "</div>" +
      '<div class="form-group"><label class="form-label">Nechta ishlab chiqarildi? *</label>' +
      '<input type="number" id="prod-qty" min="1" max="' +
      remaining +
      '" placeholder="' +
      remaining +
      '"/></div>' +
  ...
  var rows = products
    .map(function (p) {
      ...
      return (
        ...
        fmtNum(qty) +
        "</span></td>" +
        "<td><button class=\"btn btn-sm btn-ghost\" onclick=\"showAdjustInvModal('shop','" +
        p.id +
        "','" +
        p.name +
        "'," +
...
function showEditSaleModal(logId) {
  ...
  openModal(
    mHead("✏ Sotuvni tahrirlash") +
      ...
      "</div>" +
      '<div class="form-row cols-2">' +
      '<div><label class="form-label">Miqdor *</label><input type="number" id="es-qty" min="1" value="' +
      l.quantity +
      '"/></div>' +
      '<div><label class="form-label">Narx (1 dona, so\'m)</label><input type="number" id="es-price" min="0" value="' +
      (l.pricePerUnit || 0) +
      '"/></div>' +
      "</div>" +
      '<div class="form-group"><label class="form-label">Izoh</label>' +
      '<input type="text" id="es-note" value="' +
      (l.note || "") +
      '"/></div>' +
...
function confirmDeleteSale(logId) {
  ...
  if (
    !confirm(
      '"' +
        l.productName +
        "\" sotuvini o'chirasizmi?\nInventar qayta tiklanadi.",
    )
  )
    return;
  ...
  toast("🗑 Sotuv o'chirildi, inventar tiklandi");
  renderShop();
  if (currentPage === "dashboard") renderDashboard();
}
...
function renderReports() {
  ...
  var monthly = Object.entries(r.monthly)
    .sort()
    .map(function (e) {
      return { label: e[0], val: e[1].qty, sub: fmtMoney(e[1].revenue) };
    });
  ...
  if (logs.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="7"><div class="empty"><div class="empty-icon">📋</div><p>Yozuvlar yo\'q</p></div></td></tr>';
    return;
  }
  tbody.innerHTML = logs
    ...
    .map(function (l) {
      return (
        ...
        "</strong></td>" +
        "<td>" +
        (l.pricePerUnit > 0 ? fmtMoney(l.pricePerUnit) : "—") +
        "</td>" +
        "<td>" +
        (l.totalPrice > 0 ? fmtMoney(l.totalPrice) : "—") +
        "</td>" +
        '<td style="font-size:12px;color:var(--ink3)">' +
        (l.note || "—") +
        (l.editedAt ? ' <span style="color:var(--amber)">✏</span>' : "") +
        "</td>" +
        "</tr>"
      );
    })
  ...
  el.innerHTML = archives
    .map(function (a) {
      return (
        ...
        '<span style="font-size:11px;color:var(--ink3)">Arxivlangan: ' +
        fmtDate(a.archivedAt) +
        "</span>" +
        '<button class="btn btn-sm btn-primary" onclick="showArchiveDetail(\'' +
        a.id +
        "')\">📋 Ko'rish</button>" +
        '<button class="btn btn-sm btn-red" onclick="confirmDeleteArchive(\'' +
        a.id +
        "')\">🗑</button>" +
        "</div>" +
        "</div>" +
        '<div class="g4" style="margin-top:12px">' +
        '<div class="archive-stat"><div class="archive-stat-val">' +
        fmtNum(a.summary.totalSold) +
        '</div><div class="archive-stat-lbl">Sotilgan</div></div>' +
        '<div class="archive-stat"><div class="archive-stat-val">' +
        fmtMoney(a.summary.totalRevenue) +
        '</div><div class="archive-stat-lbl">Daromad</div></div>' +
        '<div class="archive-stat"><div class="archive-stat-val">' +
        fmtNum(a.summary.totalProduced) +
        '</div><div class="archive-stat-lbl">Ishlab chiqarilgan</div></div>' +
        '<div class="archive-stat"><div class="archive-stat-val">' +
        fmtNum(a.summary.totalTransferred) +
        '</div><div class="archive-stat-lbl">Yuborilgan</div></div>' +
        "</div>" +
        "</div>"
      );
    })
...
function showArchiveDetail(archiveId) {
  ...
  var salesRows = Object.entries(a.summary.salesByProduct)
    .map(function (e) {
      return (
        "<tr><td>" +
        e[1].name +
  ...
  var wsRows = products
    .map(function (p) {
      return (
        ...
        p.name +
        "</span><strong>" +
        (a.snapshots.wsInventory[p.id] || 0) +
        "</strong></div>"
      );
    })
    .join("");

  var shopRows = products
    .map(function (p) {
      return (
        '<div style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid var(--border);font-size:13px">' +
        "<span>" +
        ...
        p.name +
        "</span><strong>" +
        (a.snapshots.shopInventory[p.id] || 0) +
        "</strong></div>"
      );
    })
    .join("");
  ...
  openModal(
    mHead("📋 " + a.label + " — Arxiv hisoboti") +
      ...
      '<div class="archive-stat-inline"><strong>' +
      fmtNum(a.summary.totalSold) +
      "</strong><span>Sotilgan</span></div>" +
      '<div class="archive-stat-inline"><strong>' +
      fmtMoney(a.summary.totalRevenue) +
      "</strong><span>Daromad</span></div>" +
      '<div class="archive-stat-inline"><strong>' +
      fmtNum(a.summary.totalProduced) +
      "</strong><span>Ishlab chiqarilgan</span></div>" +
      '<div class="archive-stat-inline"><strong>' +
      fmtNum(a.summary.totalTransferred) +
      "</strong><span>Yuborilgan</span></div>" +
      "</div>" +
      '<div class="divider"></div>' +
      '<div class="card-title" style="margin:12px 0 8px">💰 Mahsulot bo\'yicha sotuv</div>' +
      (salesRows
        ? '<table class="data-table"><thead><tr><th>Mahsulot</th><th>Miqdor</th><th>Daromad</th></tr></thead><tbody>' +
          salesRows +
          "</tbody></table>"
      ...
      '<div class="form-row cols-2">' +
      '<div><div style="font-size:12px;font-weight:700;color:var(--ink3);margin-bottom:6px">SEH</div>' +
      wsRows +
      "</div>" +
      '<div><div style="font-size:12px;font-weight:700;color:var(--ink3);margin-bottom:6px">DO\'KON</div>' +
      shopRows +
      "</div>" +
      "</div>" +
...
function showGroupPasswordModal(groupId) {
  ...
  openModal(
    mHead('🔑 "' + g.name + '" paroli') +
      ...
      '<input type="text" id="grp-pw" value="' +
      (g.password || "") +
      '" placeholder="Kamida 3 ta belgi"/></div>' +
