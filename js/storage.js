...
var Storage = (function () {
  ...
  function deleteGroup(id) {
    if (getTasks().find((t) => t.groupId === id && t.status === "active"))
      return {
        ok: false,
        msg: "Bu guruhda faol topshiriqlar bor. Avval topshiriqlarni o'chiring.",
      };
    _set(
      KEYS.GROUPS,
  ...
  function updateTask(id, fields) {
    ...
    if (fields.quantity !== undefined) {
      ...
      if (nq < tasks[idx].produced)
        return {
          ok: false,
          msg: `Miqdor ${tasks[idx].produced} tadan kam bo'lishi mumkin emas.`,
        };
    ...
    if (fields.pricePerItem !== undefined)
      tasks[idx].pricePerItem = parseFloat(fields.pricePerItem) || 0;
  ...
  function deleteTask(id) {
    ...
    if (task.produced > 0)
      return {
        ok: false,
        msg: `${task.produced} ta allaqachon ishlab chiqarilgan. O'chirib bo'lmaydi.`,
      };
    _set(
      KEYS.TASKS,
  ...
  function editSaleLog(logId, newQty, newPricePerUnit, newNote) {
    ...
    const log = logs[idx];
    if (log.type !== "sale")
      return { ok: false, msg: "Faqat sotuv yozuvlarini tahrirlash mumkin." };
    ...
    if (newShopQty < 0)
      return {
        ok: false,
        msg:
          "Do'kon inventarida yetarli mahsulot yo'q (" +
          currentQty +
          " ta mavjud).",
      };
    ...
    logs[idx] = {
      ...
      totalPrice: newQty * newPricePerUnit,
      note: newNote || logs[idx].note,
  ...
  function deleteSaleLog(logId) {
    ...
    const log = logs[idx];
    if (log.type !== "sale")
      return { ok: false, msg: "Faqat sotuv yozuvlarini o'chirish mumkin." };
    const shopInv = getShopInventory();
  ...
  function _buildArchive(monthKey, logs) {
    ...
    sl.forEach((l) => {
      if (!salesByProduct[l.productId])
        salesByProduct[l.productId] = {
          name: l.productName,
          qty: 0,
          revenue: 0,
        };
  ...
  function archiveMonth(monthKey) {
    ...
    if (archives.find((a) => a.monthKey === monthKey))
      return {
        ok: false,
        msg: _monthLabel(monthKey) + " oyi allaqachon arxivlangan.",
      };
    const logs = getLogs();
    const ml = logs.filter((l) => l.date.startsWith(monthKey));
    if (ml.length === 0)
      return { ok: false, msg: "Bu oyda hech qanday yozuv topilmadi." };
  ...
  function getArchiveById(id) {
    return getArchives().find((a) => a.id === id) || null;
  }
  function deleteArchive(id) {
    _set(
      KEYS.ARCHIVES,
      getArchives().filter((a) => a.id !== id),
    );
    return { ok: true };
  }
  ...
  function getReportSummary() {
    ...
    logs
      .filter((l) => l.type === "sale")
      .forEach((l) => {
        if (!salesByProduct[l.productName])
          salesByProduct[l.productName] = { qty: 0, revenue: 0 };
